...
bool check_full(const Game *game)
{
    for (int i = 0; i < tam; i++)
    {
        ...
        {
            if (game->board[i][j] == 0)
            { // percorre todo array e ao encontrar espaco livre retorna 0
                return false;
            }
        }
    }
...
void save_game(Game *game, const char *filename)
{
    ...
    for (int i = 0; i < tam; i++)
    {
        ...
        fprintf(file, "%d ", *game->prev_board[i]);
        fprintf(file, "%d\n", *game->backup_board[i]);
    }
...
void load_game(Game *game, const char *filename)
{
    ...
    if (game->board)
    { // Liberar memória dos tabuleiros se já foi alocada
        for (int i = 0; i < tam; i++)
        {
            free(game->board[i]);
            free(game->prev_board[i]);
...
void move_board(Game *game, Direction dir)
{
    ...
    case UP:
        for (j = 0; j < tam; j++)
        {
            ...
            {
                if (game->board[i][j] != 0)
                { // Encontrou um número diferente de 0
                    k = i;
